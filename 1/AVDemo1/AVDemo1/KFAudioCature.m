...
@implementation KFAudioCature

- (instancetype)initWithConfig:(KFAudioConfig *)config {
    self = [super init];
    if (self) {
        _config = config;
        _captureQueue = dispatch_queue_create("com.kky.audioCapture", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (void)startRunning {
    ...
    dispatch_async(self.captureQueue, ^{
        ...
        if (startStatus != noErr) {
            // 捕捉并回调开始采集时错误
            [weakSelf callBackError:[NSError errorWithDomain:NSStringFromClass([KFAudioCature class]) code:startStatus userInfo:nil]];
        }
    });
...
- (void)stopRunning {
    ...
    dispatch_async(self.captureQueue, ^{
        if (weakSelf.audioCaptureInstance) {
            // 停止采集
            OSStatus status = AudioOutputUnitStop(weakSelf.audioCaptureInstance);
            if (status != noErr) {
                // 捕捉并回调停止采集时的错误。
                //[weakSelf];
            }
...
- (void)setupAudioCaptureInstance:(NSError **)error {
    ...
    // 5. 设置实例的属性: 音频参数 如 数据格式、声道数、采样位深、采样率
    ...
    asbd.mBytesPerPacket = asbd.mFramesPerPacket * asbd.mBytesPerFrame; // 每个包子节数
...
+ (CMSampleBufferRef)sampleBufferFromAudioBufferList:(AudioBufferList)buffers
                                      ...
                                      inNumberFrames:(UInt32)inNumberFrames
                                                desc:(AudioStreamBasicDescription)desc {
    ...
    time /= info.denom;
    // pts
    CMTime persentationTime = CMTimeMake(time, 1000000000.0f);
    // 对于音频 ptshe dts 是一样的
    CMSampleTimingInfo timing = {CMTimeMake(1, desc.mSampleRate), persentationTime, persentationTime};
    ...
    // 4 创建 CMBlockBuffer 实例 其中数据拷贝自AudioBufferList 并将 CMBlockBuffer 实例关联到 CMSampleBuffer 实例
...
static  OSStatus audioBufferCallBack(void *inRefCon,
    ...
    @autoreleasepool {
        KFAudioCature *capture = (__bridge  KFAudioCature *)inRefCon;
        if (!capture) {
            return -1;
        }
        
        // 1 创建AudioBufferList空间 用来接受采集数据
        AudioBuffer buffer;
        buffer.mData = NULL;
        buffer.mDataByteSize = 0;
        // 采集的时候设置了数据格式 kAudioFormatLinearPCM，声道交错格式 所以即使是双声道这里也设置 mNumberChannels 为 1
        ...
        // 3 数据封装及回调
        if (status == noErr) {
            CMSampleBufferRef sampleBuffer = [KFAudioCature sampleBufferFromAudioBufferList:buffers inTimeStamp:inTimeStamp inNumberFrames:inNumberFrames desc:capture.audioFormat];
            
            if (capture.sampleBufferOutputBlock) {
                capture.sampleBufferOutputBlock(sampleBuffer);
            }
